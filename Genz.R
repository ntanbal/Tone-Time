...
genz_plot <- ggplot(data = genz_result, aes(x = Year)) +
  ...
  labs(x = "Year", y = "Sentiment Score", title = "Sentiment Scores for genz Generation") +
  scale_x_continuous(breaks = unique(genz_result$Year), labels = unique(genz_result$Year)) +
  theme_minimal() +
  theme(
...
ggplot(head(genz_word_table, 10), aes(x = word, y = Frequency, fill = sentiment)) +
  ...
  labs(title = "Top 10 Words in genz Generation Songs", x = "Word", y = "Frequency") +
